  ...
  return (
    <section className="mt-20">
      ...
      <article className="border-l-2 pl-4 border-navy-400 mb-8">
        {description?.map((descObj, index) =>
          descObj.linkText && descObj.href ? (
            <p key={index} className="mb-4">
              ...
              <strong>
                <Link
                  className="text-brown-800 font-bold underline mt-4"
                  href={descObj.href}
                >
                  {descObj.linkText}
                </Link>
        ...
        {subHeadingText && (
          <h3 className="mt-4 text-xl mb-2 font-bold text-dark-blue">
            {subHeadingText}
          </h3>
        )}
        {subHeadingDescription && (
        ...
        {specificProjects && (
          <ul className="ml-4 list-disc py-4">
            {specificProjects.map((project, index) => (
              <li key={index} className="mb-2">
                {project?.textStart ? project?.textStart + " " : ""}
                {project.href ? (
                  <strong>
                    <Link
                      className="text-brown-800 font-bold underline mt-4"
        ...
        <article className="border-l-2 pl-4 border-navy-400">
          <ul className="ml-4 list-disc py-4">
            {links.map((linkObj, index) => (
              <li key={index} className="mb-2">
                {linkObj?.textStart ? linkObj?.textStart + " " : ""}
                <strong>
                  <Link
                    className="text-brown-800 font-bold underline mt-4"
                    href={linkObj.href}
