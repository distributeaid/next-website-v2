  ...
  const ImageGallery = ({ fund }: FundProps) => {
    ...
    return (
      <Container
        ...
        px="4"
      >
        <Grid display="grid" columns={{ initial: "2",  sm:"4" }} rows={{ initial: "2",  sm:"1" }} gap="4" flow="column" width="100%" mb="64px">
            <>
                {gallery_images && gallery_images.map((image, index) => (
                    <Box key={index}>
                        <Image
                            src={image || "/images/donate/ukraine-response-gallery-03.png"}
                            alt={""}
                            height={0}
                            width={0}
                            sizes={"100vw"}
